...
data = pd.read_csv("./Ex13_Pandas_Missing_Data.csv", sep=";")
display(data.head(5))

display(Markdown("##### Showing the Missing Values"))
missing = pd.merge(pd.Series(data.isna().mean(), name="% Missing"), pd.Series(data.isna().sum(), name="# Missing"), left_index=True, right_index=True).sort_values("% Missing", ascending=False)
...
data["weight"] = data.groupby(["mpg"], group_keys=False)["weight"].apply(lambda g : g.fillna(g.mean()))
...
data["displacement"] = data.groupby(["cylinders", "horsepower"], group_keys=False)["displacement"].apply(lambda d : d.fillna(d.mean()))
data["displacement"] = data.groupby(["cylinders"], group_keys=False)["displacement"].apply(lambda d : d.fillna(d.mean()))
