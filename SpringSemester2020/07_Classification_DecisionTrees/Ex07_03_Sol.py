...
data = pd.read_csv("./Ex07_03_Data.csv")
display(data.head(5))
...
data = pd.concat([data.drop("hotel", axis=1), pd.get_dummies(data["hotel"])], axis=1)
display(data.head(5))
...
data = pd.concat([data.drop("meal", axis=1), pd.get_dummies(data["meal"])], axis=1)
display(data.head(5))

display(Markdown("- *distribution_channel*"))
data = pd.concat([data.drop("distribution_channel", axis=1), pd.get_dummies(data["distribution_channel"], prefix="dist_ch")], axis=1)
display(data.head(5))

display(Markdown("- *deposit_type*"))
data = pd.concat([data.drop("deposit_type", axis=1), pd.get_dummies(data["deposit_type"])], axis=1)
display(data.head(5))

display(Markdown("- *customer_type*"))
data = pd.concat([data.drop("customer_type", axis=1), pd.get_dummies(data["customer_type"])], axis=1)
display(data.head(5))
