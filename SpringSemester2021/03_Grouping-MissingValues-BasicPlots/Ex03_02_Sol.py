...
releaseDates = games.groupby("Platform")["Year_of_Release"]
...
scores = games.groupby(["Platform", "Genre", "Publisher"])["Critic_Score"]
games["Critic_Score"] = scores.apply(lambda g : g.fillna(g.median()))
display(games.isna().mean())

display(Markdown("Genre & Publisher"))
scores = games.groupby(["Genre", "Publisher"])["Critic_Score"]
games["Critic_Score"] = scores.apply(lambda g : g.fillna(g.median()))
display(games.isna().mean())

display(Markdown("Genre"))
scores = games.groupby("Genre")["Critic_Score"]
games["Critic_Score"] = scores.apply(lambda g : g.fillna(g.median()))
display(games.isna().mean())
