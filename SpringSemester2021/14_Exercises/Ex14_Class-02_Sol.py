...
data = pd.read_csv("./Ex14_Class-02_Data-Train.csv")
display(data.head(5))
display(data.info())
...
data_holdout = pd.read_csv("./Ex14_Class-02_Data-Holdout.csv")
...
data_churn = pd.DataFrame(model_best.predict_proba(data_holdout.drop("CustomerID",axis=1)))
data_churn = data_churn.rename(columns={0:"Stays", 1:"Churn"})
data_churn = pd.concat([data_churn[["Churn", "Stays"]], data_holdout], axis=1)
