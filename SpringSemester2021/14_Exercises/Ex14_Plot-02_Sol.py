...
prices = pd.read_csv("./Ex14_Plot-02_Data-TechnologyIndex.csv")
display(prices.head(5))

display(Markdown("###### Preprocessing Prices"))
prices[prices.columns.drop("Country")] = prices[prices.columns.drop("Country")].apply(lambda c: c.str.replace("$","").str.replace(",","").str.strip()).astype(float)
display(prices.head(5))
display(prices.info())

display(Markdown("###### Adding Continent"))
countries = pd.read_csv("./Ex14_Plot-02_Data-Countries.csv")
data = pd.merge(prices, countries[["Country_Name", "Continent_Name"]], how="left", left_on="Country", right_on="Country_Name").drop("Country_Name", axis=1)
data = data.rename(columns={"Continent_Name" : "Continent"})
display(data.head(5))

display(Markdown("###### Adding Population"))
population = pd.read_csv("./Ex14_Plot-02_Data-Population.csv")
data = pd.merge(data, population[["Location", "PopTotal"]], how="left", left_on="Country", right_on="Location").drop("Location", axis=1)
display(data.head(5))
...
minmax = [macwin_parts["Count"].max(), parts["Count"].min()]
...
continents = dict(zip(continents,range(len(ids))))
